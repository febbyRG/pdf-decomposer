...
- **Multi-page articles finally group** via two new continuity signals: a discourse-connective opening ("Meanwhile, ...") and matching running-head kickers (the clean composer stashes top-margin text as `metadata.runningHeadText`; PdfPageComposer merges consecutive pages whose kicker token sets are identical, multi-token, and document-rare). wa.pdf alone gains 14 correct article groups (3-page features, 2-page spreads) the composer never formed before.
